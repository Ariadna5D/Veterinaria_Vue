...
<template>
    <div class="form-wrapper">
        <form @submit="alEnviar" class="flex-col" style="gap: 1.5rem;">
            ...
            <div class="grid-2">
                <div class="flex-col">
                    <!-- Formulario validado por vue -->
                    <label>{{ t('ingreso.formulario.campos.tNombreMascota') }}</label>
                    <InputText v-model="nombre" @blur="blurNombre" :class="{ 'p-invalid': errorNombre }" />
                    <small class="error-msg">{{ errorNombre }}</small>
                </div>

                <div class="flex-col">
                    <label>{{ t('ingreso.formulario.campos.tEspecie') }}</label>
                    <Select :placeholder="t('ingreso.formulario.seleccionar')" v-model="especie" :options="especies"
                        optionLabel="label" optionValue="value" class="w-full" />
                    <small class="error-msg">{{ errorEspecie }}</small>
                </div>
            </div>

            <div class="grid-2">
                <div class="flex-col">
                    <label>{{ t('ingreso.formulario.campos.tDueno') }}</label>
                    <InputText v-model="dueno" @blur="blurDueno" :class="{ 'p-invalid': errorDueno }" />
                    <small class="error-msg">{{ errorDueno }}</small>
                </div>

                <div class="flex-col">
                    <label>{{ t('ingreso.formulario.campos.tTelefono') }}</label>
                    <InputText v-model="telefono" @blur="blurTelefono" type="tel"
                        :class="{ 'p-invalid': errorTelefono }" />
                    <small class="error-msg">{{ errorTelefono }}</small>
                </div>
            </div>

            <div class="flex-col">
                <label>{{ t('ingreso.formulario.campos.tNotas') }}</label>
                <Textarea v-model="notas" rows="3" class="w-full" />
            </div>

            <button type="submit" class="btn-primary w-full">{{ t('ingreso.formulario.accion') }}</button>
        </form>
    </div>
</template>
